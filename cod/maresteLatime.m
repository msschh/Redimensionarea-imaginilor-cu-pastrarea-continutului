function imgRez = maresteLatime(img,numarPixeliLatime,metodaSelectareDrum,ploteazaDrum,culoareDrum)

E = calculeazaEnergie(img);
drumuri = selecteazaDrumuriVerticale(img,E,metodaSelectareDrum,numarPixeliLatime,ploteazaDrum,culoareDrum);
imgRez = uint8(zeros(size(img,1),size(img,2)+numarPixeliLatime,size(img,3)));
for i = 1:size(img,1)
   k = 1;
   j = 1;
   while k + j - 1 <= size(img,2)+numarPixeliLatime
       while drumuri(i, j) ~= 0
           if k + j - 1 == 1
           ...
           elseif k + j - 1 == size(img,2)+numarPixeliLatime
